...
// Query: *[_type == "bigcommerceProduct" && _id == $id][0]{    _id,    _type,    "title": select(      defined(seo.title) => seo.title,      store.title    ),    "description": select(      defined(seo.description) => seo.description,      store.descriptionHtml    ),    "image": select(      defined(seo.image.asset) => seo.image.asset->url + "?w=1200&h=630&dpr=2&fit=crop",      defined(store.previewImageUrl) => store.previewImageUrl    ),    "price": coalesce(store.salePrice, store.price),    // Variants are their own synced documents joined on the product's entityId,    // not an array of references — BigCommerce webhook payloads are unordered,    // so the sync stores the join key rather than a reference array. The    // was/now pair keeps the OG card's discount maths unchanged: a synced    // variant marked down carries the pre-markdown figure in `price`.    "variants": *[      _type == "bigcommerceProductVariant"      && store.productEntityId == ^.store.entityId      && store.isDeleted != true    ]{      "price": coalesce(store.salePrice, store.price),      "compareAtPrice": store.price    },    "dominantColor": seo.image.asset->metadata.palette.dominant.background,    "seoImage": seo.image.asset->url + "?w=1200&h=630&dpr=2&fit=max",    "logo": *[_type == "settings"][0].logo.asset->url + "?w=80&h=40&dpr=3&fit=max&q=100",    "siteTitle": *[_type == "settings"][0].siteTitle,    "date": coalesce(store.createdAt, _createdAt)  }
...
// Query: *[_type == "bigcommerceCategory" && _id == $id][0]{    _id,    _type,    "title": select(      defined(seo.title) => seo.title,      store.title    ),    "description": select(      defined(seo.description) => seo.description,      store.descriptionHtml    ),    "image": select(      defined(seo.image.asset) => seo.image.asset->url + "?w=1200&h=630&dpr=2&fit=crop",      defined(store.imageUrl) => store.imageUrl    ),    "dominantColor": seo.image.asset->metadata.palette.dominant.background,    "seoImage": seo.image.asset->url + "?w=1200&h=630&dpr=2&fit=max",    "logo": *[_type == "settings"][0].logo.asset->url + "?w=80&h=40&dpr=3&fit=max&q=100",    "siteTitle": *[_type == "settings"][0].siteTitle,    "date": coalesce(store.createdAt, _createdAt)  }
...
// Query: *[_type == "navbar" && _id == "navbar"][0]{    _id,    columns[]{      _key,      _type == "navbarColumn" => {        "type": "column",        title,        links[]{          _key,          name,          icon,          description,          "openInNewTab": url.openInNewTab,          "href": select(      url.type == "internal" => coalesce(        url.internal->slug.current,        "/collections/" + url.internal->store.slug.current      ),      url.type == "external" => url.external,      url.type == "email" => "mailto:" + url.email,      url.type == "product" => "/products/" + url.product->store.slug.current,      url.href    )        }      },      _type == "navbarLink" => {        "type": "link",        name,        description,        "openInNewTab": url.openInNewTab,        "href": select(      url.type == "internal" => coalesce(        url.internal->slug.current,        "/collections/" + url.internal->store.slug.current      ),      url.type == "external" => url.external,      url.type == "email" => "mailto:" + url.email,      url.type == "product" => "/products/" + url.product->store.slug.current,      url.href    )      },      _type == "collectionGroup" => {        "type": "collectionGroup",        title,        // Both fields are weak references to a synced category and both render        // as /collections/{slug}, so both carry the explorer's clauses — the        // dropdown is one more surface that can link a category        // queryCollectionPaths never prerenders.        //        // array::compact for the dangling half: CollectionGroupDropdown maps        // collectionLinks and reads .slug off each entry, so a null from a        // reference whose target the sync has not written yet is a TypeError,        // not a missing row. featuredProductsBlock and the explorer compact the        // identical shape.        "collectionLinks": array::compact(collectionLinks[@->store.isDeleted != true && @->store.isVisible == true]->{          _id,          "slug": store.slug.current,          store{            title,            imageUrl          }        }),        // A single reference cannot take a filter, so the same two clauses go        // in a select. No default: the dropdown already treats a missing        // collectionProducts as "no View all link" rather than a dense field.        "collectionProducts": select(          collectionProducts->store.isDeleted != true && collectionProducts->store.isVisible == true => collectionProducts->{            _id,            "slug": store.slug.current,            store{              title            }          }        )      }    },      buttons[]{    text,    variant,    _key,    _type,    "openInNewTab": url.openInNewTab,    "href": select(      url.type == "internal" => coalesce(        url.internal->slug.current,        "/collections/" + url.internal->store.slug.current      ),      url.type == "external" => url.external,      url.type == "email" => "mailto:" + url.email,      url.type == "product" => "/products/" + url.product->store.slug.current,      url.href    ),  },  }
...
// Query: *[_type == "bigcommerceProduct" && store.slug.current == $handle && store.isDeleted != true][0]{    _id,    _type,    "slug": store.slug.current,    "title": store.title,      body[]{    ...,      _type == "block" => {    ...,      markDefs[]{    ...,      ...customLink{    openInNewTab,    "href": select(      type == "internal" => coalesce(        internal->slug.current,        "/collections/" + internal->store.slug.current      ),      type == "external" => external,      type == "email" => "mailto:" + email,      type == "product" => "/products/" + product->store.slug.current,      "#"    ),  },    _type == "linkInternal" => {      "href": reference->slug.current,    },    _type == "linkExternal" => {      "href": url,      "openInNewTab": newWindow,    },    _type == "linkEmail" => {      "href": "mailto:" + email,    },  }  },  _type == "image" => {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },    "caption": caption  },  _type == "imageWithProductHotspots" => {    _type,    _key,    image{  "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }},    showHotspots,      productHotspots[]{    _key,    x,    y,      productWithVariant{    // Null once the sync tombstones the product, which drops the whole spot:    // `ProductHotspotsImage` already bails on a spot with no product. The    // alternative is a card that links to a PDP which 404s, and the repo has    // settled that argument the other way once already — see the comment on the    // flatMap in `components/product/featured-cards.ts`. The sync soft-deletes,    // so without this the document is still there and still projects happily.    "product": select(      product->store.isDeleted != true => product->{        _id,        "slug": store.slug.current,        store{          title,          previewImageUrl,          // A synced product carries one price, not a min/max range. The          // hotspot card still reads a range, so it is built here rather than          // reshaping the card: a marked-down product keeps the pre-markdown          // figure in `price`, which makes it the top of the range.          "priceRange": {            "minVariantPrice": coalesce(salePrice, price),            "maxVariantPrice": price          }        }      }    ),    variant->{      _id,      store{        title,        price,        imageUrl      }    }  }  }  },  _type == "accordion" => {    _type,    _key,    groups[]{      _key,      title,      body[]{        ...,        _type == "block" => {          ...,            markDefs[]{    ...,      ...customLink{    openInNewTab,    "href": select(      type == "internal" => coalesce(        internal->slug.current,        "/collections/" + internal->store.slug.current      ),      type == "external" => external,      type == "email" => "mailto:" + email,      type == "product" => "/products/" + product->store.slug.current,      "#"    ),  },    _type == "linkInternal" => {      "href": reference->slug.current,    },    _type == "linkExternal" => {      "href": url,      "openInNewTab": newWindow,    },    _type == "linkEmail" => {      "href": "mailto:" + email,    },  }        }      }    }  },  _type == "callout" => {    _type,    _key,    text  }  }  }
...
// Query: *[_type == "bigcommerceProduct" && defined(store.slug.current) && store.isDeleted != true && store.isVisible == true].store.slug.current
export type QueryProductPathsResult = Array<string | null>;

// Source: ../../packages/sanity/src/query.ts
// Variable: queryCollectionPaths
// Query: *[_type == "bigcommerceCategory" && defined(store.slug.current) && store.isDeleted != true && store.isVisible == true].store.slug.current
